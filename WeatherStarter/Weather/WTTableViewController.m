...
{
    //primero se define la cadena de donde vendra el json
    //se forma la url y se hace el request
    NSString *string = [NSString stringWithFormat:@"%@weather.php?format=json",BaseUrlString];
    NSURL *url = [NSURL URLWithString:string];
    NSURLRequest * request = [NSURLRequest requestWithURL:url];
    
    //se hace el request con el objeto tipo request que se creo
    AFHTTPRequestOperation *operation = [[AFHTTPRequestOperation alloc] initWithRequest:request];
    //fija el resultado del response y usa tipo json
    operation.responseSerializer = [AFJSONResponseSerializer serializer];
    //invoca el servicio haciendo uso del delegado
    //si recibe una respuesta adecuada la despliega, sino
    //lanza un error
    [operation setCompletionBlockWithSuccess:^(AFHTTPRequestOperation *operation, id responseObject){
        ...
        UIAlertView *message = [[UIAlertView alloc] initWithTitle:@"Error recibiendo el clima"
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    ...
    switch (section) {
        case 0:{
            return 1;}
        case 1: {
            NSArray *justATest = [self.weather justATest];
            NSLog(@"%@",justATest);
...
- (void) actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex{
    ...
    if (buttonIndex == 0) {
        [manager GET:@"weather.php" parameters:parameters success:^(NSURLSessionDataTask *task, id responseObject){
            ...
            UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"Error obteniendo weather" message:[error localizedDescription] delegate:nil cancelButtonTitle:@"OK" otherButtonTitles: nil];
            [alertView show];
        }];
    }
